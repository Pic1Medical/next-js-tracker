...
function EditForm({ entry }: Readonly<{ entry: EntryType }>) {
  ...
  useEffect(()=>{
    return tracker?.register(()=>{
      const confirm = window.confirm("Are you sure you wish to delete this Category item?");
      if(!confirm) return;
      client.models.Category.delete({ id: entry.id })
        .then((result) => {
          if (!result.data) throw "Failed to delete Category item, try again...";
          else {
            toast.success("Successfully deleted Category item.");
            router.back();
          }
        })
      .catch(handleError);
