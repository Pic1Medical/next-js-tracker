...
export default function () {
  ...
  async function onSubmit(data: CreateFormSchema) {
    ...
    await (async () => {
      ...
      {
        // 2) If we don't have an existing location, add one.
        const result = await client.models.Location.create({
          ...data,
        });
        if (!!result.data) {
          router.replace(
            "/client/tracker/location/details?id=" + result.data.id
          );
          return;
        } else {
          const message = "Failed to create new Location, try again...";
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          >
            <legend className="flex gap-2 items-center bg-sidebar-primary text-sidebar-primary-foreground rounded-lg border border-muted px-2 py-1">
              Create Location <MapPinIcon size={18} />
            </legend>
